...
Notable approaches using distance measures, such as the Normalized Compression Distance (NCD)~\citep{elmanarelbouanani_authorship_2014}, or the Compression-Based Classification (CBC)~\citep{bevendorff_divergence_based_2020,bevendorff_overview_2024}, rely on compression algorithms such as PPM, LZW, and GZIP. 
...
While powerful, DetectGPT is limited by its white-box scenarios %(access to model probabilities) 
and is vulnerable to paraphrasing or detecting unseen models~\citep{Wu_ODD_challenges_2025}.
...
Instead, we construct frequency-based n-gram vectors and apply input vector similarity metrics.
