...
\begin{itemize}
    ...
    \item We further analyse prompt design, its influence on paraphrase length as a confounding variable (cf.~\Cref{sec:prompt_impact_res}/~\ref{enum:rq1} and \ref{enum:rq2}) and on quantitative metrics adapted from summarisation and translation tasks (hereafter paraphrasing scores; cf.~\Cref{sec:comp_paraphrases}/~\ref{enum:rq2}).
    ...
    using pairs of human-authored texts in the traditional \ac{av} setting (cf.~\Cref{sec:imp_gen_res}/~\ref{enum:rq3}).
