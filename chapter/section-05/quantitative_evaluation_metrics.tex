...
This allows for the same word having different embeddings depending on its context.
...
For recall calculation, each token embedding $r_i$ in the reference $r$ is matched to a token embedding $c_j$ in the candidate $c$.
Analogous, for computing precision, each token embedding $c_i$ in the candidate $c$ is matched to a token embedding $r_j$ in the reference $r$.
Matching is carried out in a greedy fashion.
...
The cosine similarity between dense vector representations $v_a, v_b$ of two documents $a,b$ is computed in \autoref{eq:cosine_sim}.
Values range from $-1$ to $1$.~\citep{thongtan_cosine_sim_19,zhang_bertscore_2020}.
$-1$ means $v_a, v_b$ show in opposite directions, $0$ means they are unrelated, and $1$ means they show in the same direction.
