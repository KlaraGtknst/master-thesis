...
Evaluating the Pearson correlation to human judgement on \num{10000}~samples from web pages from the Wayback Machine and the Common Crawl dataset, \citet{anantha_pearson_metrics_2021}\ found that none of \ac{bleu}, \ac{rouge}, and METEOR have a higher Pearson correlation to human judgement than $0.64$.
...
As per \citet{gohsen_captions_2023}, we compute semantic similarity between transformer based text embeddings.

\paragraph{\acs{bert}Score.}
\acs{bert}Score computes similarity between \ac{bert} embeddings of candidate text $c$ and reference text $r$~\citep{hanna_fine_grained_2021}. 
...
The cosine similarity between vector representations $x$ and $y$ of two documents is defined in \Cref{eq:cosine_sim}. 
Cosine similarity values range from $-1$ to $1$~\citep{thongtan_cosine_sim_19,zhang_bertscore_2020}, where $-1$ indicates that $x$ and $y$ point in opposite directions, $0$ indicates orthogonal vectors, and $1$ indicates that they point in the same direction. 
Following \citet{gohsen_captions_2023}, we compute our document embedding vectors using a \ac{sbert} model.
...
In order to transform document $d$ to document $d'$, (1) the outgoing flow from word $i$ equals $d_i$, i.e.\ $\sum_{j}T_{ij}=d_i$, and (2) the incoming flow to word $j$ must match $d'_j$, i.e.\ $\sum_{i}T_{ij}=d'_j$.
...
Then, the average syntactic similarity $\diameter_{syn}$ and the average semantic similarity $\diameter_{sem}$ are calculated.
The syntactic metrics considered are \ac{rouge}-1, \ac{rouge}-L, and \ac{bleu}.
Semantic measures include \ac{wms}, \ac{bert}Score, and cosine similarity of the \ac{sbert} embeddings.
Finally, $\Delta_{sem,syn}$ is defined as in \Cref{eq:gohsen_delta}, i.e.\ the difference of semantic and syntactic average similarity~\citep{gohsen_captions_2023}.
\begin{equation}
