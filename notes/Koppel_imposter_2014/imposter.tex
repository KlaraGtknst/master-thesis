...
\begin{algorithm}
    ...
    \Procedure{IsSameAuthor}{$X$, $Y$, $\sigma^*$}
        ...
        \State Initialize $scores = \{\}$ \Comment{Matching scores for reference documents}
    
        \ForAll{$d \in \{X, Y\}$}  
            \State $d' \gets \{X, Y\} \setminus \{d\}$ 
            \Comment{Non-reference document $d'$}
            \State $\mathcal{I}_d \gets \text{GenerateImposters}(d)$ 
            \Comment{$m$ imposter documents for reference $d$}
    
            \State $scores[d] \gets 0$ 
            \Comment{Count $d'=$ top match}
