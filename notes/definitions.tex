...
\begin{definition}
    [Stylometry]
    Linguistic research area, which refers to the (statistical) analysis of authorial/ literally style \citep{elmanarelbouanani_authorship_2014,neal_surveying_2018}.
...
\begin{definition}
    ...
    Opposed to other text categorisation tasks, features do not include content \citep{koppel_authorship_2004,gohsen_task_oriented_2024}.
\end{definition}
...
\begin{definition}
    ...
    without necessarily identifying the author \citep{elmanarelbouanani_authorship_2014}.
\end{definition}
...
\begin{definition}
    ...
    In terms of machine learning, this is a multi-class, single-label text categorisation task 
...
\begin{definition}
    ...
    The analysis attempts to categorise individual texts (e.g. whether an email was authored by an email address) \citep{abbasi_writeprints_2008}.
\end{definition}
...
\begin{definition}
    ...
    Identity-level analysis' categorisation is based on all texts written by that identity.
...
\begin{table}[tbp]
    ...
    Document length analysis & \multicolumn{1}{l|}{Uniform length} & \multicolumn{1}{l|}{Lexical character features} & One-class density estimation & Heuristic voting \\
    ...
    Analysis of issuing institution & \multicolumn{1}{l|}{Text element boundaries} & \multicolumn{1}{l|}{Syntactical features} & One-class reconstruction & Human inspection \\
     ...
     & \multicolumn{1}{l|}{Stylistic boundaries} & \multicolumn{1}{l|}{Language modelling} &  & Qsum \\
...
\begin{definition}
    ...
    Supervised techniques for stylometric analysis require (author-)class labels for categorisation.
...
\begin{definition}
    ...
    Unsupervised techniques make categorisations with no prior knowledge of author classes.
...
\begin{definition}
    [text distortion]
    This domain-adversarial method substitutes out-of-vocabulary items with asterisks $*$ \citep{tyo_state_2022}.
...
\begin{definition}
    ...
    optimise the model's hyperparameters on a second disjoint selection of the dataset (i.e.\ validation set),
...
\begin{definition}
    ...
    \citet{bevendorff_divergence_based_2020} explicitly states that the goal is to maximise the paraphrases' style difference to the reference text.
\end{definition}
...
\begin{definition}
    [\acl{rl}]
    \ac{rl} trains agents to take actions in an environment to maximise a cumulative reward \citep{zhou_paraphrase_2021}.
\end{definition}
...
\begin{definition}
    [External validity]
    External validity is the extent to which the study results can be generalised 
...
\begin{definition}
    ...
    reused as the starting point (i.e.\ fine-tuning) for a model on a second task (i.e.\ downstream task) \citep{palivela_optimization_2021}.
\end{definition}
...
\begin{definition}
    ...
    It is the exponential average of the negative log-likelihood of a sequence generated by the model:
